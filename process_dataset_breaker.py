...
def process_video(idx, file_name, file_ext):
    ...
    while cap.isOpened():
        ...
        if not skip_frame:
            ...
            # Write to file
            with open(os.path.join(final_dir, f"breaker_{idx}_{frame_ind}.txt"), "w") as f:
                f.write(out_str)

            # Write image to file
            cv2.imwrite(os.path.join(final_dir, f"breaker_{idx}_{frame_ind}.jpg"), frame)
...
if __name__ == "__main__":
    ...
    for filename in os.listdir(root):
        ...
        if (file_ext == ".MOV" or file_ext == ".mp4") and "BREAKER" in file_name:
            # Add to list
